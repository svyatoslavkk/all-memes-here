...
import { Gif, FavGif } from "../../types/types";
...
export default function Card({ gif }: CardProps) {
  ...
  const isFavoriteGif = myData?.favoriteGifs.some(
    (favGif: FavGif) => favGif.id === gif.id,
  );
        ...
        setUsers((prevUsers) => {
          ...
          if (userIndex !== -1) {
            const updatedUserData = {
              ...myData,
              favoriteGifs: isFavoriteGif
                ? myData.favoriteGifs.filter(
                    (favGif: FavGif) => favGif.id !== gif.id,
                  )
