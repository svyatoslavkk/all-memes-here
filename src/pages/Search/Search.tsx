...
export default function Search() {
  ...
  const {
    data: searchGifs,
    isLoading: searchGifsLoading,
    isError: searchGifsError,
  } = useGetSearchGifsQuery(searchTerm);
  const {
    data: searchStickers,
    isLoading: searchStickersLoading,
    isError: searchStickersError,
          ...
          {!searchGifsLoading && searchTerm.length <= 0 && (
            <div className="column-content">
              <h2 className="big-header">
                Find any {activeButton === "Gifs" ? "gifs" : "stickers"} you
                want...
              </h2>
            </div>
          )}
          {searchGifsError && <p>Error fetching search results</p>}
          {searchStickersError && <p>Error fetching search results</p>}
